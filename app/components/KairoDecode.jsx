...
const SEED_GLOSSARY = [
  { term: "Agent", emoji: "🤖", definition: "An AI system that can take actions on its own — not just answer questions, but actually do things: browse the web, write code, send messages, manage files.", examples: [{ label: "Claude agent browsing the web", url: "https://www.anthropic.com/claude" }, { label: "AutoGPT (early popular agent)", url: "https://github.com/Significant-Gravitas/AutoGPT" }], deepDive: ["What's the difference between an AI assistant and an AI agent — what can one do that the other can't?", "Where might I first come across AI agents in everyday tools or apps I already use?", "Do AI agents make their own decisions, or do they always follow instructions?"], smartLines: ["An agent can book a meeting or pull a report — things a chatbot would just tell you how to do yourself.", "Saying 'I have an agent on it' sounds better than 'automated script', and is usually more accurate."], tag: "Behaviour", seeded: true },
  { term: "Agentic", emoji: "⚡", definition: "Adjective for AI behaviour that's goal-driven and multi-step — the AI works out what to do next, rather than just responding to a single question.", examples: [{ label: "Anthropic on agentic AI", url: "https://www.anthropic.com/research/building-effective-agents" }], deepDive: ["What does 'agentic' actually mean — is it just a fancy word for 'automatic'?", "If I'm using an AI tool that does multiple steps on its own, is that agentic — how would I know?", "Does agentic AI mean the AI is more powerful, or just that it works differently?"], smartLines: ["The difference between agentic and a chatbot: one gives you instructions, the other just does it.", "Our workflow is fully agentic — the AI handles the steps, we handle explaining it to everyone else."], tag: "Behaviour", seeded: true },
  { term: "Model", emoji: "🧠", definition: "The AI brain itself — software trained on vast amounts of text that reads your input and generates a response. GPT-4, Claude, and Gemini are all different models with different strengths.", examples: [{ label: "Anthropic model overview", url: "https://docs.anthropic.com/en/docs/about-claude/models/overview" }, { label: "OpenAI model comparison", url: "https://platform.openai.com/docs/models" }], deepDive: ["In plain terms, what is an AI model — is it more like software, or more like a brain?", "When people talk about GPT-4, Claude, or Gemini, are they all basically the same thing with different branding?", "If I'm using an AI app, am I interacting with the model directly — or is there something in between?"], smartLines: ["We switched models and output quality improved — not all models are equal for every task.", "Claude handles nuance well, GPT-4o is fast, Llama runs locally. Picking the right one matters."], tag: "Model", seeded: true },
  { term: "CLI", emoji: "💻", definition: "Command Line Interface — you type commands into a terminal window instead of clicking through an app. How developers talk to their computers, and increasingly how they use AI tools.", examples: [{ label: "Claude Code CLI", url: "https://docs.anthropic.com/en/docs/claude-code/overview" }, { label: "GitHub CLI", url: "https://cli.github.com/" }], deepDive: ["What's the difference between the CLI and a normal app — why would anyone prefer typing commands?", "Do I need to learn the CLI to use AI tools, or can I get by without it?", "Is the CLI the same as Terminal on Mac or Command Prompt on Windows?"], smartLines: ["Once you're comfortable with the CLI, it's faster than any app — the learning curve is the only real barrier.", "The CLI does exactly what you type, nothing more. That's both the appeal and the occasional problem."], tag: "Dev Tool", seeded: true },
  { term: "IDE", emoji: "🖥️", definition: "Integrated Development Environment — a powerful text editor for writing code, with tools built in to help you navigate, debug, and ship faster. Cursor and VS Code are the popular ones right now.", examples: [{ label: "Cursor (AI-native IDE)", url: "https://cursor.com" }, { label: "VS Code", url: "https://code.visualstudio.com/" }], deepDive: ["Is an IDE just a fancy text editor, or is there more to it than that?", "If I'm not a developer, would I ever need an IDE?", "What does 'AI-native' mean for an IDE like Cursor — what does it actually do differently from a normal code editor?"], smartLines: ["Switched to an AI-native IDE and haven't looked back — it handles the repetitive parts so you can focus on the interesting ones.", "Cursor and VS Code are the main options — both solid, but Cursor is built with AI at the core."], tag: "Dev Tool", seeded: true },
  { term: "Token", emoji: "🪙", definition: "The unit AI models use to measure text — roughly ¾ of a word. Every message you send and receive uses tokens, and that's what you're billed for. 1,000 tokens ≈ 750 words.", examples: [{ label: "Anthropic pricing (tokens)", url: "https://www.anthropic.com/pricing" }], deepDive: ["What exactly is a token — is it a word, a letter, or something else entirely?", "Why do AI companies charge by tokens instead of by message or by minute?", "If I'm just chatting with an AI app, do I need to worry about tokens at all?"], smartLines: ["Every message you send uses tokens — inputs and outputs both count, so longer conversations cost more.", "Knowing your token usage per request is the first step to making your AI costs predictable."], tag: "Economics", seeded: true },
  { term: "Context Window", emoji: "🪟", definition: "How much text an AI can hold in mind at once — its working memory. Bigger windows cost more but let you work with longer documents and conversations without the AI losing track.", examples: [{ label: "Claude's 200k context window explained", url: "https://www.anthropic.com/news/claude-2-1" }], deepDive: ["What does 'context window' mean in plain terms — what does the AI actually lose when it runs out of space?", "Does a bigger context window always mean a better AI, or are there trade-offs?", "Have I ever hit a context window limit without realising it — what would that look like?"], smartLines: ["The context window is why your AI forgot something you mentioned earlier in a long conversation. Entirely by design.", "We hit the limit mid-document. The AI now has very confident opinions about page one."], tag: "Economics", seeded: true },
  { term: "RAG", emoji: "📚", definition: "Retrieval-Augmented Generation — instead of relying on the model's memory, you pull in relevant documents at the moment of asking and include them with the question. Usually smarter than fine-tuning.", examples: [{ label: "What is RAG?", url: "https://aws.amazon.com/what-is/retrieval-augmented-generation/" }], deepDive: ["What does RAG actually mean in plain terms — what is it doing that a regular AI chat doesn't?", "If I'm using an AI assistant at work that knows our company documents, is that RAG?", "Is RAG something I need to understand as a user, or is it just a behind-the-scenes technical detail?"], smartLines: ["RAG means the model searches your docs before it answers — more reliable than hoping it remembers the right thing.", "We added RAG to our knowledge base. The model now knows our policies better than most people on the team."], tag: "Architecture", seeded: true },
  { term: "Prompt Engineering", emoji: "✍️", definition: "The practice of writing clear, well-structured instructions for AI models to get consistently good outputs. Small changes to how you phrase things can make a significant difference to the results.", examples: [{ label: "Anthropic prompt engineering guide", url: "https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview" }], deepDive: ["Is prompt engineering something anyone can learn, or do you need a technical background?", "If I write a longer, more detailed prompt, does that always get better results?", "Is prompt engineering becoming less important as AI models get smarter?"], smartLines: ["Good prompt engineering is invisible — bad prompt engineering starts every response with 'Certainly! Here's a numbered list.'", "Spent an afternoon on prompt engineering so the model would stop saying 'As an AI language model.' Completely worth it."], tag: "Technique", seeded: true },
  { term: "MCP", emoji: "🔌", definition: "Model Context Protocol — Anthropic's open standard for connecting AI models to external tools, data, and services. Instead of building separate connections for everything, MCP gives you one consistent way to wire things together.", examples: [{ label: "MCP introduction", url: "https://modelcontextprotocol.io/introduction" }, { label: "Claude MCP docs", url: "https://docs.anthropic.com/en/docs/mcp" }], deepDive: ["What does MCP actually do — what would using it look like in practice?", "If I'm not a developer, is MCP something I'd ever interact with directly?", "Why does AI need a special protocol to connect to other tools — can't it just use the internet like everything else?"], smartLines: ["MCP lets an AI connect to your calendar, files, or databases without a custom integration for each one.", "Think of MCP as the connector layer between your AI and the rest of your tools — one standard instead of one-off solutions."], tag: "Architecture", seeded: true },
  { term: "Hallucination", emoji: "👻", definition: "When an AI states something confidently that simply isn't true — and doesn't know it's wrong. The biggest reliability challenge in real-world AI products, and the main reason you should always verify important outputs.", examples: [{ label: "Why LLMs hallucinate", url: "https://www.ibm.com/topics/ai-hallucinations" }], deepDive: ["Why does an AI make things up — doesn't it know when it doesn't know something?", "How can I tell if an AI has hallucinated in its response?", "If I'm using AI for everyday tasks, does hallucination matter — or is it mainly a problem for serious applications?"], smartLines: ["The model cited a source so convincingly I nearly used it in a presentation. Turned out not to exist.", "We have a dedicated hallucination review step in QA now — not a sentence I expected to be writing."], tag: "Risk", seeded: true },
  { term: "Fine-tuning", emoji: "🎛️", definition: "Further training an existing model on your own data to improve it for a specific task or writing style. It works well, but it's expensive and time-consuming — better prompts or RAG usually get you there first.", examples: [{ label: "OpenAI fine-tuning guide", url: "https://platform.openai.com/docs/guides/fine-tuning" }], deepDive: ["What does it mean to fine-tune an AI — are you teaching it new things, or something else?", "Is fine-tuning something a regular person or small company could do, or is it only for big tech?", "If a company fine-tunes an AI on its own data, could that data end up training other people's AI?"], smartLines: ["We fine-tuned on our support tickets. The model now handles edge cases the base model never got right.", "Fine-tuning makes sense when you have lots of examples and need consistent outputs at scale — otherwise, start with RAG."], tag: "Architecture", seeded: true },
];
