  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 10000, background: "rgba(0,0,0,0.65)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem" }}>
      <div style={{ background: "#0d0d1c", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "18px", padding: "2rem 1.75rem", maxWidth: "300px", width: "100%", textAlign: "center" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.82rem", marginBottom: "1.5rem", lineHeight: 1.5 }}>Your initials will appear on any terms you contribute to Decode.</p>
        <input
            ...
            <div className="pt-3" style={{ borderTop: "1px solid rgba(var(--rgb),0.06)" }}>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  style={{
                    background: (!isOnline || !customQuestion.trim() || loadingCustom) ? "rgba(var(--rgb),0.08)" : "var(--btn-accent-bg)",
                    color: (!isOnline || !customQuestion.trim() || loadingCustom) ? "rgba(var(--rgb),0.3)" : "var(--btn-accent)",
                    border: "1px solid var(--btn-accent-border)",
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const translateChunk = async (chunk) => {
        ...
        const text = await callClaude(primaryModel, payload);
        try {
          return JSON.parse(text);
        } catch {
          // Malformed JSON — retry with Sonnet
          const text2 = await callClaude("claude-sonnet-4-6", payload);
...
If relevant:
{"relevant":true,"term":"Canonical Name","emoji":"single emoji","definition":"One crisp sentence — written for a founder or engineer building with AI, not a textbook. Avoid 'a mathematical system that...' style. Ground it in what this thing actually means when you encounter it in practice.","examples":[{"label":"short label","url":"https://real-url.com"}],"deepDive":["A question someone new to AI would ask to understand what this actually means in plain terms.","A question about when or why someone would realistically encounter or use this.","A question that tackles a common misconception or surprising aspect of this term."],"smartLines":["First sentence using the term naturally in a realistic context, with a touch of dry wit.","Second sentence — different angle, equally grounded."],"tag":"pick the single best tag from this list — do NOT default to Core Concept unless the term is a foundational theoretical concept (token, embedding, attention, transformer): Model=AI model or product, Training=how models are trained (fine-tuning, RLHF, LoRA, SFT, DPO, pretraining), Inference=how models run at serve time (quantization, KV cache, batching, latency), Architecture=high-level structural pattern for AI systems (RAG, mixture of experts, multi-agent), Technique=any practical method, design approach, or implementation strategy — prompting, retrieval design, chunking, fine-tuning recipes, evaluation methods, Behaviour=how a model generates or responds (output style, quirks, emergent capabilities), Economics=cost, pricing, compute, business model, Risk=failure modes with real-world consequences — hallucination, context rot, bias, jailbreaking, misuse, safety, alignment, Craft=product/design/writing skill, Core Concept=fundamental AI/ML theory only, or invent a new precise tag if none fit"}
...
IMPORTANT: Core AI/ML vocabulary is ALWAYS relevant even as a standalone word — never reject: AI, artificial intelligence, ML, machine learning, model, token, prompt, embedding, agent, inference, training, fine-tuning, parameter, weight, context, attention, transformer, dataset, benchmark, latency, throughput, pipeline, skill, skills, tool, tools, memory, retrieval, grounding.
...
Already in glossary (do not duplicate): ${allTermNames.join(", ")}`,
  ...
  return (
    <div className={`min-h-screen${!isDark ? " light-text-override" : ""}`} style={{
      ...
      "--rgb": isDark ? "255,255,255" : isMint ? "42,58,106" : "15,15,30",
      "--surface": surface,
      "--card-bg": isSpaghetti ? "rgba(12,12,22,0.85)" : isDark ? "rgba(255,255,255,0.04)" : isMint ? "rgba(240,252,246,0.82)" : "rgba(255,255,255,0.82)",
      "--card-border": isSpaghetti ? "rgba(255,255,255,0.10)" : isDark ? "rgba(255,255,255,0.09)" : isMint ? "rgba(42,58,106,0.25)" : "rgba(15,15,30,0.13)",
      ...
      "--deep-bg": isDark ? "rgba(255,255,255,0.025)" : isMint ? "rgba(42,58,106,0.04)" : "rgba(15,15,30,0.04)",
      "--deep-border": isDark ? "rgba(255,255,255,0.07)" : isMint ? "rgba(42,58,106,0.2)" : "rgba(15,15,30,0.1)",
      ...
      "--highlight-bg": isDark ? "rgba(99,102,241,0.2)" : isMint ? "rgba(42,58,106,0.1)" : "rgba(99,102,241,0.12)",
